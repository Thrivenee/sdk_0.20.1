    ...
    for step in scenario.steps.iter() {
        match step {
            ...
            Step::ScCall {
                ...
                tx,
                expect,
            } => denali_step::sc_call::execute(state, contract_map, tx_id, tx, expect),
            Step::ScQuery {
                tx_id,
                comment,
                tx,
                expect,
            } => denali_step::sc_query::execute(state, contract_map, tx_id, tx, expect),
            Step::ScDeploy {
                tx_id,
                comment,
                tx,
                expect,
            } => denali_step::sc_deploy::execute(state, contract_map, tx_id, tx, expect),
            ...
            Step::CheckState { comment, accounts } => {
                denali_step::check_state::execute(accounts, state);
            },
            Step::DumpState { .. } => {
