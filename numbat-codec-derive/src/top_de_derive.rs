    ...
    match &ast.data {
        ...
        syn::Data::Enum(data_enum) => {
            ...
            if is_fieldless_enum(data_enum) {
                ...
                let top_decode_body = quote! {
                    let mut nested_buffer = top_input.into_nested_buffer();
                    let result = match <u8 as numbat_codec::NestedDecode>::dep_decode(&mut nested_buffer)? {
                ...
                let top_decode_or_exit_body = quote! {
                    let mut nested_buffer = top_input.into_nested_buffer();
                    let result = match <u8 as numbat_codec::NestedDecode>::dep_decode_or_exit(&mut nested_buffer, c.clone(), exit) {
